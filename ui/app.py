...
from ui.theme import CSS_INJECTION
...
# Inject Pro Max UI/UX styling
st.markdown(CSS_INJECTION, unsafe_allow_html=True)

# Application Header Component
def render_header():
    st.title("Business Opportunity Finder")
    st.markdown('<p class="subtitle" style="color: #64748B; font-size: 1.1rem; margin-bottom: 2rem;">AI-Powered Discovery Engine for Premium Business Investments</p>', unsafe_allow_html=True)
...
def perform_scrape(days_back, min_price, max_price):
    """Cached scraping operation to improve performance and reduce redundant requests."""
...
@st.cache_data(ttl=3600, show_spinner=False)
def perform_ai_scoring(listings_hash, min_price, max_price, target_states, _listings):
    """Cached AI scoring operation. Uses a custom hash to ensure cache invalidation."""
    recommender = BusinessRecommender()
    return recommender.score_businesses(
        _listings,
        min_price=min_price,
        max_price=max_price,
...
def save_to_history(history_entry, export_data):
    """Saves the current run to a persistent history file."""
    ...
    # Add new entry containing metadata and the actual full export data
    entry = {
...
tab_discovery, tab_history = st.tabs(["🔍 Discovery Engine", "📚 Historical Data"])

with tab_discovery:
    # Main action
    if st.sidebar.button("🚀 Start Discovery", type="primary", use_container_width=True):

        # Progress and Skeleton loading
        ...
        try:
            # Step 1: Scrape
            status_text.markdown("🔄 **Step 1/3**: Scraping live business listings...")
            ...
            result = perform_scrape(days_back, min_price, max_price)
            ...
            if not result.listings:
                ...
                # Step 2: AI Analysis
                if use_ai:
                    status_text.markdown("🧠 **Step 2/3**: AI analyzing business fundamentals...")
                    progress_bar.progress(60)

                    # Generate a hash of dealer_ids to act as a cache key
                    listings_hash = ",".join(sorted([l.dealer_id for l in result.listings]))

                    # Always score broadly; apply UI-side category filtering to avoid empty AI results
                    recommendations = perform_ai_scoring(
                        listings_hash,
                        min_price,
                        max_price,
                        target_states,
                ...
                # Step 3: Display
                status_text.markdown("📊 **Step 3/3**: Building dashboard...")
                ...
                if result.listings:
                    ...
                    st.markdown("Download curated data for offline financial modeling or CRM import.")
                    ...
                    save_to_history(history_entry, export_data)
                    ...
                    with st.expander("📋 View Raw Feed (Top 100)", expanded=False):
                        full_rows = []
                        ...
                        st.dataframe(full_rows, use_container_width=True, hide_index=True, height=300)
...
with tab_history:
    st.subheader("📚 Historical Scrapes")
    st.markdown("Browse previously generated business recommendations and analytics.")
    ...
    if os.path.exists(history_file):
        try:
            ...
            if saved_history:
                for idx, item in enumerate(saved_history):
                    meta = item.get("metadata", {})
                    data = item.get("data", [])

                    with st.expander(f"Search at {meta.get('time', 'Unknown')} | Found: {meta.get('found', 0)} | Focus: {meta.get('filters', 'None')}"):
                        if data:
                            st.dataframe(
                                data,
                                use_container_width=True,
                                ...
                                column_config={
                                    "rank": st.column_config.NumberColumn("#", width=50),
                                    "score": st.column_config.ProgressColumn("Score", format="%f", min_value=0, max_value=100, width=100),
                                    "name": st.column_config.TextColumn("Business Name", width=250),
                                    "category": st.column_config.TextColumn("Category", width=140),
                                    "price": st.column_config.TextColumn("Price", width=100),
                                    "ai_analysis": st.column_config.TextColumn("AI Insight", width=300)
                                }
                            )
                        else:
                            st.info("No recommendations data saved for this run.")
            else:
                st.info("No history recorded yet. Run a discovery to save data!")
        except Exception as e:
